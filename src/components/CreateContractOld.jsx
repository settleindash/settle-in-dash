...
// Component for creating contracts with odds-based logic and DASH blockchain integration.
...
const CreateContract = () => {
  ...
  const [odds, setOdds] = useState(""); // Renamed from percentage
  ...
  const handleSubmit = async (e) => {
    ...
    const additionalContractCreator = Number(stake) * 0.1; // 10% additional contract for twist
    ...
    const result = await createContract({
      ...
      stake: Number(stake),
      odds: Number(odds), // Updated from percentage
      category: selectedEvent.category,
      WalletAddress: walletAddress,
      acceptanceDeadline: deadline.toISOString(),
      time: eventTime.toISOString(),
      additional_contract_creator: additionalContractCreator, // Added for twist
    });
