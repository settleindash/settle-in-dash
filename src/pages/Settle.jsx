...
const Settle = () => {
  const { contracts, settleContract, triggerTwist, loading, error } = useContracts();
  const [submitterEmail, setSubmitterEmail] = useState("");
  const [winnerChoice, setWinnerChoice] = useState("");
  const [reasoning, setReasoning] = useState("");
  const [selectedContractId, setSelectedContractId] = useState(null);

  // Filter accepted and twist contracts
  const acceptedContracts = contracts?.filter((c) => c.status === "accepted") || [];
  const twistContracts = contracts?.filter((c) => c.status === "twist") || [];

  console.log("Settle: Accepted Contracts:", acceptedContracts);
  console.log("Settle: Twist Contracts:", twistContracts);
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      ...
      <main className="max-w-7xl mx-auto p-4">
        <h2 className="text-xl font-semibold mb-4">Accepted Contracts</h2>
